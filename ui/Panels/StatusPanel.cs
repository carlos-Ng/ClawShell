...
public class StatusPanel : UserControl
{
	...
	private readonly Label _agentStatus;
	...
	private void BuildLayout()
	{
		...
		const int LEFT     = 16;
		const int STATUS_X = 160; // 右侧状态文字固定起始列（与最宽名称"确认通道"对齐，含 DPI 余量）
		...
		AddIndicatorRow(_channelDot, _channelStatus, "确认通道", LEFT, STATUS_X, y); y += ROW_STEP;
		AddIndicatorRow(_agentDot,   _agentStatus,   "Agent",    LEFT, STATUS_X, y);

		// 连接区块底部 ≈ 44 + 30*2 + 22 = 126，留 16px 间距 → 142
		// ── 当前任务 ──
		Controls.Add(MakeSectionHeader("当前任务", LEFT, 142));

		// AutoSize=true 的 Label 高度由字体决定，Y=170 留足空间
		_noTaskLabel.AutoSize  = true;
		_noTaskLabel.Location  = new Point(LEFT, 170);
		...
		// 任务卡片（有任务时显示）
		_taskCard.Location    = new Point(LEFT, 166);
	...
	private void RefreshConnection()
	{
		...
		SetIndicator(_channelDot, _channelStatus, _state.ChannelConnected, "确认通道", "已连接", "未连接");
		SetIndicator(_agentDot,   _agentStatus,   _state.AgentConnected,   "Agent",    "已连接", "未连接");
	}
	...
	{
		var color = active ? COLOR_OK : COLOR_OFF;
		dotLabel.Text      = active ? $"●  {name}" : $"○  {name}";
		dotLabel.ForeColor = color;
		statusLabel.Text      = active ? onText : offText;
